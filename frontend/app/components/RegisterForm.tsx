...
export function RegisterForm() { // Экспортируемый компонент формы регистрации
  ...
  const handleSubmit = async (e: React.FormEvent) => { // Обработчик отправки формы
    ...
    if (!username || !email || !password) { // Проверяем, что все поля заполнены
    ...
    // Пока что просто имитируем успешную регистрацию
    console.log("Регистрация:", { username, email, password }) // Выводим данные в консоль для проверки

    // Перенаправляем на главную страницу после "успешной" регистрации
    router.push("/chess") // Программная навигация на главную страницу
  }
